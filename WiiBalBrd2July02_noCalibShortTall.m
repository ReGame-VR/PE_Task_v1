...
%             [zeroX, zeroY, XMaxWeight, XMinWeight, YMaxWeight, YMinWeight] = getMaxWeight(bb); %weight shift bounds calibration
            ...
            VRdata = fopen(VRString,'a'); %Create a text file to save the collected data to at the end of the trial
            fprintf(VRdata, '\n\n\t trialNum \t time \t targetlNum \t targetTime \t Weightshift success \t button success \t random or set sequence \t X coord of target \t Y coord of target \t COPtotalpath \t ScorePerTarget \t ScorePerTrial \t CumulativeScore \n\n');
            fclose(VRdata);
...
trialNum = 42; %Set the number of trials you would like to do in a row
...
%                         set(scoreBox,'String',score)
                        ...
                        formatSpec = '\t %d \t %3.2f \t %d \t %3.2f \t %d \t %d \t %d \t %3.2f \t %3.2f \t %3.2f \t %d \t %d \t %d \n';
...
%                              targetTime = toc;
%                          end
                        fprintf(VRdata, formatSpec, j, cputime, lightNum, targetTime, wghtScs, btnScs, decide, cog(1), -cog(2), COP_length, score, trial_score, cum_score);
                        fclose(VRdata);
